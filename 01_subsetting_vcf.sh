...
tail -n+2 $window | while IFS=$'\t' read -r chr beg end locus;
    ...
    bcftools view CHRIS13K.WES.imputed.rsq03.vep.hg38.sorted.bgzip.vcf.gz -r ${window_size} -Oz -o $output_dir/${locus}.vcf.gz
    
    # remove conting from header lines due to lifting VCF (Dropping conting has consequences! Stop it!)
